    ...
    for trial in xrange(trials):
      ...
      for i in xrange(N):
        if algo == 'prior':
          ...
          trends[algo][str(N)]['ML_custom_ess'].append(custom_ess(np.exp(dump['logf_ML'][i,:]),np.exp(np.zeros(N))))
          trends[algo][str(N)]['CL_ess'].append(ess(np.ones(N)))
          trends[algo][str(N)]['CL_custom_ess'].append(custom_ess(np.exp(dump['logf_CL'][i,:]),np.exp(np.zeros(N))))
        else:
          trends[algo][str(N)]['ESS'].append(dump['ESS'][i])
          trends[algo][str(N)]['ML_ess'].append(ess(np.exp(dump['logw_ML'][i,:])))
          trends[algo][str(N)]['ML_custom_ess'].append(custom_ess(np.exp(dump['logf_ML'][i,:]),np.exp(dump['logw_ML'][i,:])))
          trends[algo][str(N)]['CL_ess'].append(ess(np.exp(dump['logw_CL'][i,:])))
          trends[algo][str(N)]['CL_custom_ess'].append(custom_ess(np.exp(dump['logf_CL'][i,:]),np.exp(dump['logw_CL'][i,:])))
...
for N in N_values:
  ...
  plt.legend((hB, hR),(r'ESS($w_\mathrm{marg}$)', r'ESS($w_\mathrm{cond}$)'),loc=2,prop={'size':10})
