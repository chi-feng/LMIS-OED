...
for i, algo in enumerate(algos):
    ...
    for j in xrange(npts):
        data = get_eig(
            "out/sweep/sweep%dd_%s%s_%03d_of_%03d" % (dim, isjoint, algo, j, npts)
        )
...
styles["exact"] = plt.Line2D(
    (0, 1), (0, 0), color=colors["exact"], linestyle="-", linewidth=0.5
)
...
def make_legend(exact=False):
    ...
    if exact:
        label_styles.append(styles["exact"])
    plt.legend(
        label_styles,
        labels,
...
# plt.ylim([0,4])
save_fig(
    "plot/sweep%dd%s_mean_%s.pdf"
    % (dim, isjoint, "noprior" if "noprior" in sys.argv else "prior")
)
...
np.savetxt(
    "plot_data/sweep%dd%s.csv" % (dim, isjoint),
    mat,
    delimiter=",",
    header=",".join(columns),
    comments="",
