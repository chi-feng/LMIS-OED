...
for algo in algos:
  ...
  for i in xrange(npts * npts):
    data = get_eig('expdesign/out/mossbauer3d_sweep%s_%s_%04d' % (suffix, algo, i))
    eig[i] = np.mean(data)
  ...
  plt.xlabel(r'$d_1$')
  plt.ylabel(r'$d_2$')
  save_fig('%s.pdf' % 'expdesign/plots/mossbauer3d_sweep%s_%s' % (suffix, algo))
  os.system('gnome-open %s.pdf' % 'expdesign/plots/mossbauer3d_sweep%s_%s' % (suffix, algo))
