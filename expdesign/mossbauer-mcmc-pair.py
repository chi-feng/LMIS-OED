...
def plot_trace(chain, burn=0):
  ...
  for i in xrange(dim):
    ...
    plt.plot(range(N-1000,N),chain[N-1000:, i], 'k-', lw=0.5)
    plt.ylabel(r'$\theta_%d$'% (i+1), rotation=0)
    plt.ylim([mean[i] - 3.0 * stdev[i],  mean[i] + 3.0 * stdev[i]])
    plt.grid()
...
def plotmatrix(chain, priors=None, names=None, burn=1000, levels=10, colormap=cm.jet):
  ...
  for i in xrange(dim):
    for j in xrange(dim):
      print 'plotting matrix subplot %d of %d (%d, %d)' % (i * dim + j + 1, dim * dim, i, j)
...
def savepair(prefix, chain1, chain2, priors, burn):
  ...
  for i in xrange(dim):
    for j in xrange(dim):
      print 'computing matrix subplot %d of %d (%d, %d)' % (i * dim + j + 1, dim * dim, i, j)
      xmin, xmax = priors[i][0][0], priors[i][0][-1]
      ymin, ymax = priors[j][0][0], priors[j][0][-1]
...
def main():
  ...
  priors = [gaussian_prior(0,1,window=1), gaussian_prior(0,0.3), gaussian_prior(0,0.3), gaussian_prior(1,0.2,window=1.5)]
  names = [r'$\text{center}$', r'$\exp(\text{width})$', r'$\exp(\text{height})$', r'$\exp(\text{offset})$']
