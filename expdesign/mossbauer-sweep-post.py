...
for algo in algos:
    ...
    for i in range(npts * npts):
        data = get_eig("expdesign/out/mossbauer_sweep%s_%s_%04d" % (suffix, algo, i))
        eig[i] = np.mean(data)
    ...
    plt.xlabel(r"$d_1$")
    plt.ylabel(r"$d_2$")
    save_fig("%s.pdf" % "expdesign/plots/mossbauer_sweep%s_%s" % (suffix, algo))
    os.system("open %s.pdf" % "expdesign/plots/mossbauer_sweep%s_%s" % (suffix, algo))
