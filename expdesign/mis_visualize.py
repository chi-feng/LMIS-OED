...
def read_dump(filename):
    ...
    for line in open(filename + "mixtureIndices"):
        data["indices"].append(list(map(int, line.split())))
    ...
    for i in range(N):
        ...
        data["postCovExact"][i] = postCovExactAgg[
            inputDim * i : inputDim * i + inputDim, :
        ]
...
priorsamp = mlines.Line2D(
    [], [], marker="o", markersize=3, markeredgewidth=0, color=gray, linestyle="None"
)
yi = mlines.Line2D(
    [], [], marker="*", markersize=10, markeredgewidth=0, color=red, linestyle="None"
)
postcont = mlines.Line2D(
    [],
    [],
    marker="o",
    markersize=10,
...
mixsamp = mlines.Line2D(
    [], [], marker="o", markersize=6, markeredgewidth=0, color=teal, linestyle="None"
)
MLcont = mlines.Line2D(
    [],
    [],
    marker="o",
    markersize=10,
...
def make_legend():
    plt.legend(
        [yi, postcont, mixcont, mixsamp, MLcont, MLsamp, unused_MLsamp],
        [
            r"$(\theta^{(i)},\eta^{(i)})$",
            r"$p(\theta,\eta|y^{(i)})$",
            r"$q_\text{post}^{(i)}$",
...
def plot_cov_ellipse(cov, pos, volume=0.5, ax=None, fc="none", ec=[0, 0, 0], a=1, lw=1):
    def eigsorted(cov):
...
def execute(i):
    ...
    artist, = plt.plot(
        data["X_prior"][i, 0], data["X_prior"][i, 1], "r*", ms=10, mew=0, zorder=10
    )
    ...
    for tup in [(1.0, 0.68)]:  # , (0.4, 0.5), (0.2, 0.75)]:
        artist = plot_cov_ellipse(
            data["postCovExact"][i],
            data["postMeanExact"][i],
            volume=tup[1],
            ec=red,
            a=tup[0],
        )
    ...
    for mi in data["indices"][i]:
        if mi == i:
            continue
        artist = plot_cov_ellipse(
            data["postCov"][mi] * 5, data["postMean"][mi], volume=0.68, ec=teal, a=0.8
        )
    ...
    for mi in data["indices"][i]:
        ...
        artist, = plt.plot(
            mixsamps[:, 0], mixsamps[:, 1], "go", color=teal, ms=4, mew=0, alpha=0.8
        )
