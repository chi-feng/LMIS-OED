...
for algo in algos:
  ...
  for i in xrange(npts * npts):
    data = get_eig('expdesign/out/mossbauer_3d_sweep_%s_%s_%04d' % (suffix, algo, i))
    eig[i] = np.mean(data)
  ...
  plt.contourf(X, Y, np.clip(eig,0,np.max(eig)), 40, cmap=matplotlib.cm.viridis,vmin=0, vmax=1.3)
  plt.colorbar()
