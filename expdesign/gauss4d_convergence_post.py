...
N_values = {
    "prior": list(map(int, np.floor(np.sqrt(evaluations / 20) + 0.5))),
    "mis": list(map(int, np.floor(np.sqrt(evaluations / 10) * 10 + 0.5))),
}

M_values = {
    "prior": list(map(int, 10 * np.sqrt(evaluations / 20) + 0.5)),
    "mis": list(map(int, np.floor(np.sqrt(evaluations / 10) / 2 + 0.5))),
}

trends = {"prior": {"name": "Prior"}, "mis": {"name": "MIS"}}
...
for i, algo in enumerate(algos):
    ...
    for j in range(len(evaluations)):
        ...
        M = M_values[algo][j]
        data = get_eig(
            "expdesign/out/gauss4d_convergence_%s_%s_%05d_%05d" % (suffix, algo, N, M)
        )
...
for index in ["N"]:
    ...
    make_legend()
    save_fig("expdesign/plots/gauss4d_convergence_mse.pdf")
    ...
    plt.xlabel(r"Model evaluations")
    save_fig("expdesign/plots/gauss4d_convergence_error.pdf")
