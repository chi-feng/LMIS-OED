...
for algo in algos:
  ...
  for i in range(5):
    ...
    handles[i] = plt.errorbar(x, y, y_err, color=colors[i], label=labels[i], lw=1, fmt='-', elinewidth=0.5, capthick=0.5, capsize=2, barsabove=True)
  ...
  plt.legend(bbox_to_anchor=(0, 1.05, 1, 1.02), loc=3, ncol=6, mode="expand", borderaxespad=0., numpoints=1, prop={'size':8})
