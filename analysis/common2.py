...
def get_eig(cursor, options):
  where = ["`%s`='%s'" % (key, value) for key, value in options.items()]
  ...
  print('got %d entries. average runtime %0.1f' %
        (len(values), np.mean(np.array(runtimes))))
  if len(values) > 0:
...
def execute(options):
  try:
    ...
    for line in output.split(b'\n'):
      ...
      if line[0:3] == b'EIG':
        # print(line[0:3], line[0:3] == b'EIG',line[6:],float(line[6:]))
        eig = float(line[6:])
        # record to database
        conn = sqlite3.connect(database)
        cursor = conn.cursor()
        ...
        sql = 'INSERT INTO runs (%s) VALUES (%s)' % (
            ','.join(['`%s`' % name for name in names]), placeholder)
  ...
  while not result.ready():
    ...
    if completed > 0:
      ...
      print('%04d/%04d %4.1f%% %s %4d/min  ETA: %3.1f\n' % (completed, len(runs),
                                                            percent, progressbar(percent / 100), rate, 1.0 / rate * result._number_left))
    sys.stdout.flush()
    time.sleep(1)
  print('\ncompleted', len(runs) - result._number_left)
  pool.close()
