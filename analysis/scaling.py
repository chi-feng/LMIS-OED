...
if 'postprocess' in sys.argv:
    ...
    for fixed in [0, 1]:
      for base_ratio in base_ratios:
        ...
        for j in range(len(N_values)):
          ...
          '''
          data = np.append(get_eig(cursor1, properties),get_eig(cursor2, properties))
  ...
  for fixed in [0, 1]:
    ...
    columns = ['N','M','W','bias','biase_err','var','var_err','mse','mse_err']
    ...
    np.savetxt("plot_data/scaling_%d_%s.csv" % (br,label), mat, delimiter=",", header=','.join(columns),comments='')
  ...
  for i, algo in enumerate(algorithms):
    for index, base_ratio in enumerate(base_ratios):
      ...
      plt.annotate(
        '$N=%d$\n$M=%dN=%d$' % (trends[algo][key]['N'][0],trends[algo][key]['target_ratios'][0]+0.5,trends[algo][key]['M'][0]),
        xy=(trends[algo][key]['W'][0], mse[0]), arrowprops=dict(arrowstyle='->'), size=9, xytext=(trends[algo][key]['W'][0]*5, mse[0]*1.5))

      plt.annotate(
        '$N=%d$\n$M=%dN$' % (trends[algo][key]['N'][-1],trends[algo][key]['target_ratios'][-1]),
        xy=(trends[algo][key]['W'][-1], mse[-1]), arrowprops=dict(arrowstyle='->'), size=9, xytext=(trends[algo][key]['W'][-1]/100, mse[-1]/1.5))
      ...
      plt.annotate(
        '$N=%d$\n$M=%dN$' % (trends[algo][key]['N'][-1],trends[algo][key]['target_ratios'][-1]),
        xy=(trends[algo][key]['W'][-1], mse[-1]), arrowprops=dict(arrowstyle='->'), size=9, xytext=(trends[algo][key]['W'][-1]/10, mse[-1]*10))
