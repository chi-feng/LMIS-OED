...
for algo in algorithms:
    ...
    for trial in range(trials):
        filename = "out/mossbauer_ess/%s_%s_%03d" % (suffix, algo, trial)
        dump = read_dump(filename)
        for i in xrange(N):
            if algo == "prior":
                trends[algo]["ML_ess"].append(ess(np.ones(M)))
                trends[algo]["ML_custom_ess"].append(
                    custom_ess(np.exp(dump["logf_ML"][i, :]), np.exp(np.zeros(M)))
                )
                trends[algo]["CL_ess"].append(ess(np.ones(M)))
                trends[algo]["CL_custom_ess"].append(
                    custom_ess(np.exp(dump["logf_CL"][i, :]), np.exp(np.zeros(M)))
                )
            else:
...
plt.legend(
    (hB, hR),
    (r"ESS($w_\mathrm{ML}$)", r"ESS($w_\mathrm{CL}$)"),
    loc=2,
    prop={"size": 10},
)
