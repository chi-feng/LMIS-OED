...
if True:
    data = {}
    columns = ['cESS_ML', 'cESS_CL', 'logML', 'logCL',
            'logMLexact', 'logCLexact', 'priorLogDensities']
        ...
        for setting in settings:
            ...
            for trial in range(trials):
                prefix = 'dump/%s_%dd/N%04d_M%04d_%04d_' % (
                    algo, options['-dim'], setting['N'], setting['M'], trial)
                print(prefix)
                if trial == 0:
                    ...
                    for column in columns:
                        info[column] = np.concatenate((info[column], np.genfromtxt(prefix + column)))
